...
export async function generateSitemaps() {
  // index-worthiness is computed in app code (anon client) — no DB view
  // / no manual SQL required.
  let spotPages = 1;
  try {
    const rows = await getIndexWorthySpots();
    spotPages = Math.max(1, Math.ceil(rows.length / SPOTS_PER_SITEMAP));
  } catch {
    spotPages = 1;
  }
...
export default async function sitemap({ id }: { id: number }): Promise<MetadataRoute.Sitemap> {
  ...
  // id >= 1 — spot chunk (index-worthy only, computed in app code)
  const chunkIndex = id - 1;
  const from = chunkIndex * SPOTS_PER_SITEMAP;
  const to = from + SPOTS_PER_SITEMAP;

  try {
    const rows = await getIndexWorthySpots();
    return rows.slice(from, to).map((s) => ({
