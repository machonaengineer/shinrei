...
import { updateReportStatusAction, updateSpotStatusAction, updateReviewStatusAction } from '@/app/actions/admin';
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-ink">通報管理</h2>
        <div className="flex flex-wrap gap-2 text-xs">
          <FilterLink href="/admin/reports" active={!searchParams.status}>すべて</FilterLink>
          {REPORT_STATUSES.map((s) => (
            <FilterLink key={s} href={`/admin/reports?status=${s}`} active={searchParams.status === s}>
              {STATUS_LABEL[s] ?? s}
            </FilterLink>
          ))}
        </div>
        ...
        columns={[
          {
            ...
            render: (r) => (
              <div className="text-xs">
                <div className="text-ink">{r.target_type}</div>
                {r.target_id && <div className="text-ink-muted">{r.target_id}</div>}
              </div>
            ),
          },
          { key: 'reason', header: '理由', render: (r) => <span className="text-ink-dim text-xs">{r.reason}</span> },
          {
            ...
            header: '本文',
            render: (r) => (
              <div className="max-w-[24rem] text-ink-dim text-xs whitespace-pre-wrap">{truncate(r.body ?? '', 200)}</div>
            ),
          },
          ...
          { key: 'created', header: '受信', render: (r) => <span className="text-xs text-ink-muted">{formatDateTime(r.created_at)}</span> },
          {
            ...
            render: (r) => (
              <div className="flex flex-wrap gap-1">
                ...
                {r.target_id && r.target_type === 'spot' && (
                  <form action={updateSpotStatusAction}>
                    <input type="hidden" name="id" value={r.target_id} />
                    <input type="hidden" name="status" value="hidden" />
                    <button className="rounded px-2 py-1 text-xs border border-accent-red/40 text-accent-red hover:bg-accent-red/10">
                      対象スポットを非公開
                    </button>
                  </form>
                )}
                {r.target_id && r.target_type === 'review' && (
