...
export const metadata: Metadata = buildMetadata({
  title: '写真ありスポット',
  description: 'ユーザー投稿による写真が登録された心霊スポット一覧。実際の現地の雰囲気を写真で確認できます。',
  path: '/ranking/photos',
});

export default async function PhotosRankingPage() {
  ...
  const url = new URL('/ranking/photos', siteUrl()).toString();
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-6">
      <header className="space-y-1">
        <nav className="text-xs text-ink-muted">
          ...
          <span className="mx-1">/</span>
          <span className="text-ink-dim">写真あり</span>
        </nav>
        <h1 className="text-2xl md:text-3xl font-bold text-ink">📸 写真ありスポット</h1>
        <p className="text-sm text-ink-dim">ユーザーが投稿した写真が登録されているスポットを、怖さ評価順で上位60件。</p>
      </header>
      <RankingList spots={spots} empty="現在、写真ありスポットはまだありません。" />
      <section className="surface-soft p-4 space-y-2">
        <h3 className="text-ink font-semibold text-sm">シェア</h3>
        <ShareButtons title="写真あり心霊スポット一覧｜心霊マップ" url={url} />
      </section>
