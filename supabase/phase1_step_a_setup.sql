...
drop policy if exists "spot-photos anon upload pending" on storage.objects;
create policy "spot-photos anon upload pending" on storage.objects
  for insert with check (
    bucket_id = 'spot-photos'
    and (storage.foldername(name))[1] = 'pending'
  );

drop policy if exists "spot-photos admin all" on storage.objects;
create policy "spot-photos admin all" on storage.objects
  for all using (bucket_id = 'spot-photos' and public.is_admin());
